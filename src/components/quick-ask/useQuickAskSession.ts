...
import { getVisibleChatModels, type CopilotSettings } from "@/settings/model";
...
export function useQuickAskSession(params: UseQuickAskSessionParams): QuickAskSessionApi {
  ...
  const resolvedModel = useMemo(() => {
    const fallbackModel = getVisibleChatModels(settings)[0] ?? null;

    try {
      const model = findCustomModel(selectedModelKey, settings.activeModels);
      if (!model.enabled || model.provider !== ChatModelProviders.OLLAMA) {
  ...
  const {
    ...
    onNoModel: () => {
      logError("No active model is configured. Please configure a model in KOS2 settings.");
      new Notice("No active model configured. Please configure a model in KOS2 settings.");
    },

...
import { getModelKeyFromModel, getVisibleChatModels, useSettingsValue } from "@/settings/model";
...
function AddProjectModalContent({ initialProject, onSave, onCancel }: AddProjectModalContentProps) {
  ...
  const visibleProjectModels = getVisibleChatModels(settings).filter(
    (model) => model.projectEnabled
  );
        ...
        >
          <ObsidianNativeSelect
            ...
            onChange={(e) => {
              ...
              const selectedModel = visibleProjectModels.find(
            ...
            options={visibleProjectModels.map((model) => ({

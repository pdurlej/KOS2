...
export const ERROR_MESSAGES = {
  INVALID_LICENSE_KEY_USER:
    "Legacy premium model configuration is not used in KOS2. Switch to local Ollama models in Settings.",
...
export function getNeedSetKeyProvider(): Provider[] {
  ...
  settings.activeModels.forEach((model) => activeProviders.add(model.provider as Provider));
  settings.activeEmbeddingModels.forEach((model) =>
    activeProviders.add(model.provider as Provider)
  );

  return Array.from(activeProviders).filter(
    (provider) => provider in ProviderInfo && !excludeProviders.includes(provider)
  );
}

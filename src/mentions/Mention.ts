...
export class Mention {
  ...
  async processYoutubeUrl(url: string): Promise<{ transcript: string; error?: string }> {
    try {
      if (!hasTranscriptApiKeyConfigured() || !getSettings().supadataApiKey) {
        return {
          transcript: "",
          error:
            "YouTube transcripts need a configured transcript API key in Workflows. The local yt-dlp + whisper path is not wired into KOS2 yet.",
        };
      }

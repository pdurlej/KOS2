...
export class MiyoClient {
   ...
   * `Authorization` uses the configured self-host API key.
   *
  ...
  private async buildHeaders(): Promise<Record<string, string>> {
    ...
    const selfHostApiKey = settings.selfHostApiKey
      ? await getDecryptedKey(settings.selfHostApiKey)
      : "";
    if (selfHostApiKey) {
      headers.Authorization = `Bearer ${selfHostApiKey}`;
    }

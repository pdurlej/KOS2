...
export function registerCommands(
   ...
   *
   * @returns True when the current workspace leaf is a markdown note
   */
  function hasActiveMarkdownNote(): boolean {
  ...
  addCheckCommand(plugin, COMMAND_IDS.KOS_ORGANISE, (checking: boolean) => {
    if (checking) {
      return hasActiveMarkdownNote();
    }

    void runWorkflowCommand(plugin, "organise");
  });

  addCheckCommand(plugin, COMMAND_IDS.KOS_NEXT_STEPS, (checking: boolean) => {
    if (checking) {
      return hasActiveMarkdownNote();
    }

    void runWorkflowCommand(plugin, "next-steps");
  });

  addCheckCommand(plugin, COMMAND_IDS.KOS_DECISION, (checking: boolean) => {
    if (checking) {
      return hasActiveMarkdownNote();
    }

    void runWorkflowCommand(plugin, "decision");
  });

  addCheckCommand(plugin, COMMAND_IDS.KOS_REVIEW, (checking: boolean) => {
    if (checking) {
      return hasActiveMarkdownNote();
    }

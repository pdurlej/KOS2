...
import { getVisibleChatModels, updateSetting, useSettingsValue } from "@/settings/model";
import {
  useStreamingChatSession,
  type StreamingChatTurnContext,
} from "@/hooks/use-streaming-chat-session";
import { ABORT_REASON, ChatModelProviders } from "@/constants";
...
function CustomCommandChatModalContent({
  ...
  const {
    ...
    onNoModel: () => {
      new Notice("No active model is configured. Please configure a model in KOS2 settings.");
      setIsLoading(false);
    },
